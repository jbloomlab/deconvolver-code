...
=head1 NAME
	...
	grid-clustalw.pl - A pure Perl API to submit jobs (or job arrays)
				    to Sun Grid Engine.
...
USAGE:
	...
	grid-clustalw.pl 
		--project 810001
		--command "clustalw-mpi seqs.fasta"
	[	--name myJob
...
EXAMPLE:
	...
	grid-clustalw.pl -p 810001 -name myJob --verbose -c "hello.pl nelson"
	
	# or to receive emails
	grid-clustalw.pl -p 810001 -name myJob -mailto naxelrod@jcvi.org --mailon abe --verbose -c "hello.pl nelson"

	# Submit a job array
	grid-clustalw.pl -p 810001 -name testarray --verbose -o /scratch/naxelrod/test -c "jobarray.sh <input dir> <output dir>"
	grid-clustalw.pl -p 810001 -name testarray --verbose -t 0-2:1 -c "/home/naxelrod/lib/perl/Grid/t/jobarray.sh /home/naxelrod/lib/perl/Grid/t /home/naxelrod/lib/perl/Grid/t/data/output"
...
# Handles submission of qsub jobs to MPI queue
my $MPI_GRID_SETTINGS = "/usr/local/sge_test/jcvi-test/common/settings.sh";
my $settings_code = `source $MPI_GRID_SETTINGS`;
die "Error: Failed to source $MPI_GRID_SETTINGS with error code: $settings_code\n"
	if $settings_code;;
...
my $command = $options{command} || &_pod("A command statement is required");


# qsub -P 08020 -pe orte 36 -l default mpirun -np 36 clustalw-mpi -infile=/usr/local/devel/ANNOTATION/naxelrod/Grid/data/fasta/MPV.fasta
